...
| CW113 | Error | File {} not found, this is case sensitive | A file path referenced in script doesn't exist. The reference is matched against the mod+vanilla file index case-insensitively by default (Windows-authored mods); with `--case-sensitive-files` (or `case-sensitive-files = true` in `cwtools.toml`), every indexed file (mod and base game, live or cache-restored) is matched by exact on-disk case so a case-mismatched reference is caught for case-sensitive filesystems (Linux/Mac). Off by default, so existing diagnostics are unchanged. | Emitted (FilepathField refs checked against the mod+vanilla file index) |
