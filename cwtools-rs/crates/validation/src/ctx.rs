...
//! parsed AST, the ruleset, the string table, the prebuilt enum map, the game,
//! and the optional type/modifier/loc indexes). Bundling it into one borrow
//! struct keeps the recursive signatures small: each call passes `&ValidationCtx`
//! plus only the genuinely per-call varying args (the current node/rules, the
//! mutable `scope_context`, and the `errors` sink).
...
pub(crate) struct ValidationCtx<'a> {
    ...
    pub(crate) enum_map: &'a HashMap<&'a str, &'a EnumDefinition>,
