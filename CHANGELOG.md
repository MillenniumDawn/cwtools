...
- The debounced per-document validation observes its task through the same panic wrapper the background reindex and the watched-file batch use, instead of its own copy of the logic, so a panic there is logged the way theirs are while the abort of an edit a newer keystroke superseded stays unlogged. `CWTOOLS_VALIDATE_PANIC_ONCE` injects a one-shot panic into the first document validation, which is what the e2e test covering it fires. (#182)
