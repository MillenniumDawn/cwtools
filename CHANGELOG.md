...
- CW226 could not fire on a localisation command chain with a dot in it. Every one of them was exempt, because nothing could tell a scripted variable from a typo, so `[?ROOT.war_suport]` read as valid however it was spelled. A `?`-marked variable read is now checked against the project's variable registry, the config's built-in `value[variable]` reads plus every name the mod sets, and only a name neither knows is reported. A chain without the `?` ends in a command or a scripted-localisation name, which no registry answers for yet, so it stays exempt; so does one reading through a variable (`[?some_var.SomeLoc]`) or through a segment the scope engine cannot resolve, such as a country tag. The pinned Kaiserreich corpus has no misspelled read, so its baseline is unchanged. (#98)
...
- **Behavioral:** a `?`-marked localisation variable read naming a variable the project never sets now reports CW226, where every multi-segment chain used to be exempt. A read through `holder:name`, a `$ARG$`-built name and a bare number stay lenient, and a workspace whose variable index is still empty reports nothing new. (#98)
