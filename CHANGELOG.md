...
- `cwtools loc` can run the loc scope and command checks. Pass `--game` and `--rules` and it loads the ruleset's scopes and links, so a `[command]` chain naming something that doesn't exist (CW226) or leaving a scope its next link doesn't accept (CW260/CW266) is reported from the standalone lint, not only from `validate`. It reads the ruleset and nothing else: no game files are discovered or indexed, so the run stays a loc scan and the chains start from an unknown scope, which means only what is wrong in every scope is reported. `validate` remains the stricter answer, since it knows the scope of the field each key is used from. Without both settings nothing changes, and one on its own warns rather than looking like it checked something. The scan also takes `--ignore-file`, `--ignore-dir` and `--loc-language`, and `loc` now reads `game`, `rules`, `ignore-files`, `ignore-dirs` and `loc-languages` from `cwtools.toml`, so a config written for `validate` covers both commands. (#95)
...
- `validate --since` now resolves the ref against `--directory` when it is run from a git hook, which is the case the flag was added for. Git exports `GIT_DIR` to its hooks and that overrides `-C`, so a pre-commit hook calling `cwtools validate --directory <mod> --since origin/main` diffed the hook's own repository instead of the mod and scoped the report to the wrong file set. The same environment is stripped from the CLI's test suite, where an inherited `GIT_DIR` made the `--since` fixture repo's `git init`, `git add` and `git commit` land on the branch being pushed. (#254)
