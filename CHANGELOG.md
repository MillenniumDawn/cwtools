...
- **Behavioral:** a rename whose edit set would touch a file outside the workspace folders (for example a definition the type index resolved into the base-game install) is now cancelled with a message naming the file and the reason, instead of writing there. A rename whose edits stay in the request's own document, including in `untitled:` documents and windows with no workspace folder, is unaffected. Symlinked files inside the workspace are skipped by the CW100 quick fix and left out of `fixAllWorkspace`; when everything fixable was skipped for that reason, the command says so instead of claiming there is nothing to fix. (#160)
