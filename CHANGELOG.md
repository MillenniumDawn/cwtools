...
- CW113's "file not found" message claimed the lookup was case-sensitive, but the file index lowercased every path on insert and lookup, so a reference that differed from the on-disk file only by case resolved anyway and the warning never fired on a case-sensitive filesystem. New `--case-sensitive-files` flag (and `case-sensitive-files = true` in `cwtools.toml`) makes the check match every indexed file, mod and base game, by exact on-disk case, so a Windows-authored mod whose `file =` paths only load on Linux/Mac gets the case mismatch flagged. Off by default, so existing diagnostics are unchanged, and the on-disk case is only collected while the flag is on (no memory cost otherwise). The base-game cache now stores each path's on-disk case (a cache-format bump, rebuilt automatically) so a cache-restored install is case-checked too. (#140)
